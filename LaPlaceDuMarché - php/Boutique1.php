...
?>    
    ...
    if(isset($_GET['category'])){
		...
		while($s=$select->fetch(PDO::FETCH_OBJ)){
			...
			<h4><?php echo $s->price; ?> €</h4>
            <h4>Stock :<?php echo $s->stock; ?> Qt ou Kg</h4>
            <?php if ($s->stock!=0){ ?><a href="panier.php?action=ajout&amp;l=<?php echo $s->stock; ?>&amp;q=1<?php echo $s->stock; ?>&amp;p=<?php echo $s->price; ?>">Ajouter au pannier</a><?php }else{echo'<h5 Stock style="color:red;">Stock épuisé !</h5>';} ?>
			<br/>
			<?php
		}	
